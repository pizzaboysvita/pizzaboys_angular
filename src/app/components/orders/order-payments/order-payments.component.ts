...
export class OrderPaymentsComponent {
  ...
  setDefaultSelectedRowForFull(): void {
    ...
    const pendingIndex = this.fullArray.findIndex(
      (row: any) => row.status === "Pending"
    );
    const indexToSelect = pendingIndex !== -1 ? pendingIndex : 0;

    this.selectedRowIndex = indexToSelect;
    this.selectedRowData = this.fullArray[indexToSelect];
  }
  ...
  setActiveTab(tab: string) {
    ...
    if (this.activeTab == "full" || this.activeTab == "items") {
      ...
      const splitfulRows = this.splitRows
        ? this.splitRows.some(
            (row: { status: string }) => row.status === "Success"
          )
        : false;
    ...
    if (this.activeTab == "full" || this.activeTab == "people") {
      this.isSplitPayment = false;
      const splitfulRows = this.paidItems
        ? this.paidItems.some(
            (row: { status: string }) => row.status === "Success"
          )
        : false;
  ...
  allPaymentsSuccessful(): boolean {
    return (
      this.fullArray.length > 0 &&
      this.fullArray.every(
  ...
  fullAddPayment() {
    ...
    if (this.paymentAmount > this.remaining) {
      this.toastr.warning(
        "Entered amount exceeds the remaining balance!",
        "Warning"
      );
      return;
    }
  ...
  appendCashNumber(num: any) {
    this.cashpaymentAmount = Number(
      this.paymentAmount.toString() + num.toString()
    );
  }
  ...
  confirmCashPayment() {
    ...
    if (this.activeTab === "full" && this.checkAllPaymentsSuccess()) {
      ...
      this.checkAllItemsPaymentsSuccess()
    ) {
  ...
  hasAnySuccess(): boolean {
    const fullSuccess = this.fullArray?.some(
      (row: any) => row.status === "Success"
    );
    const splitSuccess = this.splitRows?.some(
      (row: any) => row.status === "Success"
    );
  ...
  hasSplitSuccess(): boolean {
    const paidItems = this.paidItems?.some(
      (row: any) => row.status === "Success"
    );
    const splitSuccess = this.splitRows?.some(
      (row: any) => row.status === "Success"
  ...
  hasItemsSuccess(): boolean {
    const paidSuccess = this.paidItems?.some(
      (row: any) => row.status === "Success"
    );
    const fullSuccess = this.fullArray?.some(
      (row: any) => row.status === "Success"
