    ...
    },
    {
      ...
      cellRenderer: (params: any) => {
        ...
        select.addEventListener("change", (event) => {
          ...
          const parsedValue =
            newValue === "1" ? 1 : newValue === "0" ? 0 : newValue;
...
delete
  ...
  updateStatus(dishStatusId: any, rowData: any) {
    ...
    this.apis.patchStatusApi(reqbody).subscribe(
      (response: any) => {
  ...
  getmenuList() {
    ...
    this.apis
      .getApi(`${AppConstants.api_end_points.menu}?store_id=${-1}`)
      .subscribe((data: any) => {
        if (data) {
          data.data.forEach((item: any) => {
            item.status =
              item.status == 1 ? "Active" : item.status == 0 ? "In-Active" : "";
          });
  ...
  storeNameData(data: any) {
    ...
    const storeName = this.storeList.find(
  ...
  onConfirm(data: any) {
    ...
    this.apis
      .postApi(AppConstants.api_end_points.menuV2, formData)
      .subscribe((data: any) => {
        if (data) {
          console.log(data);
  ...
  search() {
    console.log(
      this.menuItemsSortingList,
      this.menuSearchForm.value,
      this.menuSearchForm.value.store
    );

    if (
      this.menuSearchForm.value.menuName !== "" ||
      this.menuSearchForm.value.status !== "" ||
      this.menuSearchForm.value.store !== -1
    ) {
      this.menuItemsList = this.menuItemsSortingList.filter((store: any) => {
        return (
          (!this.menuSearchForm.value.store ||
            store.store_id
              .toString()
              .includes(this.menuSearchForm.value.store.toString())) &&
          (!this.menuSearchForm.value.menuName ||
            store.name
              .toLowerCase()
              .includes(this.menuSearchForm.value.menuName.toLowerCase())) &&
          (!this.menuSearchForm.value.status ||
            store.status
              .toLowerCase()
              .includes(this.menuSearchForm.value.status.toLowerCase()))
        );
      });
    } else {
