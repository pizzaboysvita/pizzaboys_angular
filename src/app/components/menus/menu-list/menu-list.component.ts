...
export class MenuListComponent {
  ...
  getMenuCategoryDishData() {
    ...
    const menuApi = this.apiService.getApi(AppConstants.api_end_points.menu + '?store_id=' + this.selectedStoreId);
    const categoryApi = this.apiService.getApi(`/api/category?store_id=` + this.selectedStoreId);
    const dishApi = this.apiService.getApi(AppConstants.api_end_points.dish + '?store_id=' + this.selectedStoreId);
    const comboDishApi = this.apiService.getApi(
      '/api/dish?store_id=' + this.selectedStoreId + '&typeOfDish=combo'
    );
  ...
  onConfirm(data: any) {
    const reqboy = {
      ...
      "store_id": this.dishDetails.store_id
    }
