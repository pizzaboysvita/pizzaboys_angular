...
delete
  ...
  updateStatus(dishStatusId: any, rowData: any) {
    ...
    this.apiService.patchStatusApi(reqbody).subscribe(
      (response: any) => {
    ...
    this.apiService
      .getApi(
        AppConstants.api_end_points.menu +
          "?store_id=" +
          this.categoriesForm.value.store
      )
      .subscribe((data: any) => {
        if (data) {
          ...
          res.categories.forEach((categorie: any) => {
            categorie.status =
              categorie.status == 0
                ...
                ? "Active"
                : "--";
            // categorie.hide_category_in_POS = categorie.hide_category_in_POS = 1
            //   ? "Hide in POS"
            //   : categorie.hide_category_in_POS == 0
            //   ? "Show in POS"
            //   : "--";
          });
  ...
  onCellClicked(event: any): void {
    ...
    if (action === "view") {
      ...
      if (
        ...
        this.categoryRowData.store_id &&
        !Array.isArray(this.categoryRowData.store_id)
      ) {
        this.categoryRowData.store_id = [this.categoryRowData.store_id];
      }
  ...
  insertCategory(type: any) {
    ...
    modalRef.componentInstance.storeList = this.storeList;
  ...
  onConfirm() {
    ...
    this.apiService
      .postApi(AppConstants.api_end_points.categoryV2, formData)
      .subscribe((data: any) => {
        console.log(data);
        if (data.code == 1) {
          this.modelRef.close();
  ...
  menuNameData(data: any): string {
    ...
    const match = this.menuItemsList.find(
      (item: any) => item.dish_menu_id === data
    );
