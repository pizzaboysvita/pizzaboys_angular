...
@Component({
  ...
  imports: [NgbNavModule, CardComponent,ReactiveFormsModule,CommonModule,NgSelectModule],
...
export class AddStaffComponent {
  ...
  managementSections = [
    {
      ...
      key: 'restaurant',
      permissions: ['Create', 'Dashboard', 'Orders_board_View', 'Orders - List View', 'Orders - Delete','Bookings','Bookings - Delete','Customers','Menus','Menus - Images','Settings - Systems','Settings - Services','Settings - Payments','Settings - Website','Settings - Integrations','Billing','Reports'],
  ...
  constructor(private fb: FormBuilder,private apis:ApisService,private router:Router) {}
    ...
    this.applyPreset('Manager')
      ...
      phone: ['',[Validators.required, Validators.pattern(/^\+?\d[\d\s]{7,13}$/)
]],
...
this.rolesId=preset.role_id;
    switch (this.selectedPreset) {
        ...
        this.setSectionPermissions({
          restaurant: ['Create', 'Dashboard', 'Orders board View', 'Orders - List View', 'Orders - Delete','Bookings','Bookings - Delete','Customers','Menus','Menus - Images','Settings - Systems','Settings - Services','Settings - Payments','Settings - Website','Settings - Integrations','Billing',"Reports"],
          pos: ['Orders','Takings'],
          website: ['Create','Edit'],
 ...
 storeList(){
      this.apis.getApi(AppConstants.api_end_points.store_list).subscribe((data)=>{
        if(data){
          console.log(data)
...
this.apis.postApi(AppConstants.api_end_points.staff,formData).subscribe((data:any)=>{
  if(data.code ==1){
    console.log(data)
